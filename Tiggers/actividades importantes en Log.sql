...
BEGIN
    ...
    IF v_entidad = 'Modem' THEN
        ...
        ELSIF TG_OP = 'UPDATE' THEN
            ...
            ELSE
                -- No registrar actualizaciones menores
                RETURN NEW;
            END IF;
        ELSIF TG_OP = 'DELETE' THEN
    ...
    VALUES (
        ...
        v_detalle,
        CASE 
            WHEN TG_OP = 'DELETE' THEN 1 -- Usuario sistema para eliminaciones
            ELSE COALESCE(NEW."responsableId", NEW."userId", 1) -- Tomar el ID de usuario del registro si existe
        END,
        NOW()
    );
